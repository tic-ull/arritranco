...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if len(args) == 2:
            ...
            redes_parsed = self._parse_file(networks_data, ['comunicaciones.red', 'comunicaciones.datosred',
                                                            'comunicaciones.maquinared'])
            ...
            for pk in datosred.keys():
                if not pk in maquinared.keys():
                    ...
                    datosred[pk]['maquina'] = maquinas[maquinared[pk]['maquina']][
                        'nombre']  #nombre de la máquina para buscarla en el inventario actual.
            # Redes
            ...
            for dr in datosred.values():
                ...
                if m:
                    ...
                    print "Maquina %s para asociar a la ip %s, no encontrada en inventario actual" % (
                        dr['maquina'], dr['ip'])
        ...
        planificaciones = self._parse_file(backup_planificacion_data, ['backups.planificacion', ])[
            'backups.planificacion']
        patrones = self._parse_file(backup_patron_data, ['backups.patron', ])['backups.patron']
        filenamepatterns = self._parse_file(backup_filenamepatterns_data, ['backups.tiposdefichero', ])[
            'backups.tiposdefichero']
        redes_parsed = self._parse_file(networks_data,
                                        ['comunicaciones.red', 'comunicaciones.datosred', 'comunicaciones.maquinared'])
        redes = redes_parsed['comunicaciones.red']
        datosred = redes_parsed['comunicaciones.datosred']
        maquinared = redes_parsed['comunicaciones.maquinared']
        ...
        parse_data = self._parse_file(hardware_data,
                                      ['hardware.fabricante', 'hardware.tipohardware', 'hardware.modelohw',
                                       'hardware.armario', 'hardware.hardware'])
        ...
        inventory_parse_data = self._parse_file(inventario_data, ['inventario.servidor', 'inventario.modeloprocesador',
                                                                  'inventario.procesador', 'inventario.discoduro',
                                                                  'inventario.sistemaoperativo', 'inventario.maquina'])
        ...
        for dr in datosred.values():
            ...
            if m:
                ...
                print "Maquina %s para asociar a la ip %s, no encontrada o DOWN en inventario actual" % (
                    dr['maquina'], dr['ip'])
