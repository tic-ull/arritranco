...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for line in reader:
            ...
            # Create routing zone
            print (_prefix, _name)
            zone, created = RoutingZone.objects.get_or_create(prefix=_prefix, name=_name,
                                                              bluevlan_prefix=_bluevlan_prefix,
                                                              public_nets=_public_nets,
                                                              cajacanarias_nets=_cajacanarias_nets, slug="")
            self.stdout.write("RoutingZone: %s. Created: %s\n" % (zone, created))

            # Create routing building
            building, created = NetworkedBuilding.objects.get_or_create(name=_name, slug="", area=0, campus=camp,
                                                                        routingzone=zone)
